...
export default function GridView3D() {
    const { readings, gridHealth, triggerInstability, instabilityActive, pageFeeds } = useSimulation();
    const feed = pageFeeds?.gridView3D;
    const feedReadings = feed?.readings || readings;
    const feedGridHealth = feed?.gridHealth || gridHealth;
    const feedInstabilityActive = feed?.instabilityActive ?? instabilityActive;
    const healthBorderClass = !feedGridHealth
        ? "border-border"
        : feedGridHealth.healthScore >= 85
            ? "border-green-500"
            : feedGridHealth.healthScore >= 70
                ? "border-yellow-400"
                : feedGridHealth.healthScore >= 50
                    ? "border-orange-500"
                    : "border-red-500";
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className={`inline-flex h-9 w-9 items-center justify-center rounded-md border border-border text-foreground transition-colors ${feedInstabilityActive ? "bg-red-600 hover:bg-red-600/90" : "bg-zinc-800 hover:bg-zinc-700"}`}
                >
                    <Siren className="h-4 w-4" />
                </button>
            </div>
            <div className={`relative overflow-hidden rounded-xl border-2 bg-card ${healthBorderClass}`} style={{ height: "500px" }}>
                <GridScene readings={feedReadings} />
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                {feedReadings.map((/** @type {GridReading} */ node) => (
